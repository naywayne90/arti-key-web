...
const EspaceDG = () => {
  ...
  const fetchRequests = async () => {
    try {
      const data = await workflowService.getRequestsByRole('DG');
      setRequests(data);
      setError(null);
    } catch (err) {
  ...
  const handleApprove = async (requestId: string, comment: string) => {
    try {
      await workflowService.approveRequest(requestId, comment, 'DG');
      await fetchRequests();
    } catch (err) {
  ...
  const handleReject = async (requestId: string, comment: string) => {
    try {
      await workflowService.rejectRequest(requestId, comment, 'DG');
      await fetchRequests();
    } catch (err) {
